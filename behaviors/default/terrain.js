...
//console.log("Constructing hillside");
	...
	this.isFog = false;
        ...
        this.waterLevel = this.heightFieldHeight * 0.45;
        this.waterGlobal = 1.795;
        this.inWater = false;
        this.fogColor = new THREE.Color( 0.7, 0.6, 1.3 );
        this.grassColor = new THREE.Color( 0.0, 1.0, 0.0, 0.0);
        this.waterColor = new THREE.Color(0.6, 0.7, 0.85);
        this.fogDist = this.scaleHill*this.grassPatchRadius * 20.0;
        this.grassFogDist = this.scaleHill*this.grassPatchRadius * 0.5;

        this.height = -0.430*this.heightFieldHeight/2; // where is the terrain?

        const scene = this.service("ThreeRenderManager").scene;
	 scene.fog = new THREE.Fog(this.fogColor.getHex(), 0.05, this.fogDist);
